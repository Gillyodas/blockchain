...
public class TaoBangCapSinhVienCommandHandler : IRequestHandler<TaoBangCapSinhVienCommand, Result<TaoBangCapSinhVienCommandResult>>
{
    ...
    public async Task<Result<TaoBangCapSinhVienCommandResult>> Handle(TaoBangCapSinhVienCommand request, CancellationToken cancellationToken)
    {
        ...
        {
            _logger.LogError(ex, "Lỗi đồng thời khi tạo bằng cấp cho sinh viên. Lỗi: {ErrorMessage}", ex.Message);
            return Result<TaoBangCapSinhVienCommandResult>.Failure(ApplicationError.ConcurrencyError);
        }
        catch(RepositoryException ex)
        {
            _logger.LogError(ex, "Lỗi khi truy cập repository trong quá trình tạo bằng cấp cho sinh viên. Lỗi: {ErrorMessage}", ex.Message);
            return Result<TaoBangCapSinhVienCommandResult>.Failure(ApplicationError.RepositoryError);
        }
        catch(OperationCanceledException ex)
        {
            _logger.LogWarning(ex, "Giao dịch bị hủy khi tạo bằng cấp cho sinh viên. Lỗi: {ErrorMessage}", ex.Message);
            return Result<TaoBangCapSinhVienCommandResult>.Failure(ApplicationError.Cancelled);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Lỗi không xác định khi tạo bằng cấp cho sinh viên. Lỗi: {ErrorMessage}", ex.Message);
            return Result<TaoBangCapSinhVienCommandResult>.Failure(ApplicationError.UnknownError);
        }
    }
}
